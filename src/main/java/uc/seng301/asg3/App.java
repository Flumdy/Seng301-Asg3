...
public class App {
  ...
  public static void main(String[] args) {
    ...
    int quantity;
    HollowEggFactory hollowFactory  = new HollowEggFactory();
    StuffedEggFactory stuffedFactory = new StuffedEggFactory(new ArrayList<>(context.allFillings.values()));
    ...
    StrategyContext strategy = new StrategyContext(decideStrategy(packagingType), quantity, hollowFactory, stuffedFactory);
    PreparingOrder order = new PreparingOrder(hollowFactory,
       stuffedFactory, strategy);
  ...
  private static StrategyBase decideStrategy(PackagingType packagingType) {
    if (packagingType.equals(PackagingType.MIXED_BOX)) {
      return new MixedBoxStrategy();
    } else if (packagingType.equals(PackagingType.REGULAR_BOX)) {
      return new RegularBoxStrategy();
    } else if (packagingType.equals(PackagingType.MIXED_HOLLOW_EGG)) {
      return new MixedHollowEggStrategy();
    } else {
      return new RegularHollowEggStrategy();
    }
  }
}
