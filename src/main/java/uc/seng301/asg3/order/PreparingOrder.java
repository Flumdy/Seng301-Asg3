...
public class PreparingOrder extends Order {
   ...
   */
  public PreparingOrder(HollowEggFactory hollowEggFactory, StuffedEggFactory stuffedEggFactory, StrategyContext strategy) {
  ...
  public void prepare() {
    ...
    if (PackagingType.isHollowEggPackaging(packagingType)) {
      packaging.addChocolateEgg(produceEgg(hollowEggFactory, strategy.isValidChocolate(chocolateType), false));
    }
    ...
    for (int i = 0; i < quantity; i++) {
      ...
      CompletableFuture.supplyAsync(() ->
          produceEgg(strategy.getValidFactory(stuffed), strategy.isValidChocolate(chocolateType), withAlcohol(containsAlcohol)), executor)
  ...
  private ChocolateEgg produceEgg(ChocolateEggFactory factory, ChocolateType type,
    ...
    return factory.createChocolateEgg(type, containsAlcohol, strategy);
  }
